...
float Encoder::getSpeedLeft()
{
    ...
    if (micros() - lastTime > m_timeout) return 0.0f;
...
int8_t Encoder::getLeftDirection()
{
    return leftDirection;
}

int8_t Encoder::getRightDirection()
{
...
void Encoder::init()
{
    ...
    attachInterrupt(digitalPinToInterrupt(lhEncoderA), leftEncoderEvent, CHANGE);
    attachInterrupt(digitalPinToInterrupt(rhEncoderA), rightEncoderEvent, CHANGE);
}